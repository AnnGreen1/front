...
#### Axios 是什么?
Axios（[https://axios-http.com/zh/](https://axios-http.com/zh/)） 是一个基于 [promise]() 网络请求库，作用于[node.js]() 和浏览器中。 它是 [isomorphic]() 的(即同一套代码可以运行在浏览器和node.js中)。在服务端它使用原生 node.js `http` 模块, 而在客户端 (浏览端) 则使用 XMLHttpRequests。
...
```
当使用 `catch`，或者传递一个[rejection callback]()作为 `then` 的第二个参数时，响应可以通过 `error` 对象被使用，正如在[错误处理]()部分解释的那样。
### 默认配置
您可以指定默认配置，它将作用于每个请求。
