...
const handleFileSelect = (event) => {
  ...
  addLog('info', `选择了 ${files.length} 个文件`)
  files.forEach((file) => {
    addLog('info', `文件: ${file.name} (${formatFileSize(file.size)})`)
  })
}
...
const startUpload = async () => {
  ...
  try {
    ...
    addLog('error', `上传过程中发生错误: ${error.message}`)
  } finally {
...
const uploadSingleFile = async (file, fileIndex) => {
  try {
    ...
    addLog('info', `初始化成功，UploadId: ${uploadId}`)
    ...
    for (let partNumber = 1; partNumber <= totalParts; partNumber++) {
      ...
      try {
        ...
        updateProgress(fileIndex, partNumber, totalParts, uploadedBytes, file.size)

        addLog('info', `分片 ${partNumber}/${totalParts} 上传成功`)
      } catch (error) {
        if (error.name === 'AbortError') {
          addLog('warning', `文件 ${file.name} 上传被取消`)
          return
        }
        throw error
      }
    ...
    addLog('success', `文件 ${file.name} 上传完成`)
...
const updateProgress = (fileIndex, currentPart, totalParts, uploadedBytes, totalBytes) => {
  const progress = uploadProgress[fileIndex]
  if (!progress) return
  ...
  progress.speed = formatFileSize(speed) + '/s'
  progress.remainingTime = formatTime(remainingTime)
}
