# 创建自定义滚动指令
### 基本的示例
我们可能很多次想为网站的滚动事件添加一些行为，尤其是动画。已有的做法很多，但是代码和依赖最少的方式可能就是使用一个自定义指令创建一个钩子，在特定的滚动事件之后作处理。
```js
...
[Scrollmagic](http://scrollmagic.io/) 拥有一个非常丰富的生态系统，同时附带了很好的文档和演示以便大家浏览。这些特效包括且不仅限于诸如视差滚动、级联固定、章节切换和响应式持续时间等。